...
let questions=[],qIndex=0,correct=0,answered=false,start=Date.now(),skillBreakdown={};
function skillFor(q){const n=q.name;if(['Blue Ridge','Piedmont','Coastal Plain','Ridge and Valley','Appalachian Plateau'].includes(n))return 'physical-regions';if(['Brasstown Bald'].includes(n))return 'mountains-relief';if(['Chattahoochee River','Savannah River','Flint River','Ocmulgee River'].includes(n))return 'rivers';if(['Lake Lanier','Buford Dam'].includes(n))return 'lakes-reservoirs';if(n.includes('I-')||n==='Macon')return 'transportation';if(n.includes('Airport')||n==='Port of Savannah'||n==='Savannah'||n==='Atlanta'||n==='Columbus')return 'map-reasoning';return 'map-reasoning';}
const challengeBank=[
